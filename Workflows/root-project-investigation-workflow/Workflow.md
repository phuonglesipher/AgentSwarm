...
an `investigate` pass followed by a strict reviewer subgraph. The investigation
node uses a tool-capable Codex run when available so it can inspect the host
project, read the relevant files, and strengthen the brief with direct evidence
before writing the markdown handoff. Senior scoring and approval now live in
the dedicated `root-project-investigation-reviewer-workflow` subgraph so the
review logic can evolve independently while the parent workflow keeps control
