...
LOOP_SPEC = QualityLoopSpec(
    loop_id="optimize-investigation-review",
...
MANDATORY_VERIFICATION_ACTION = (
    "Run one more investigation pass that independently re-validates the performance hypothesis with fresh evidence, "
    "profiling data, call frequency analysis, or a read-only measurement before final handoff."
)
...
def _get_review_criteria(state: ReviewerState) -> tuple[tuple, ...]:
    criteria = state.get("review_criteria")
    if criteria:
        return tuple(tuple(c) for c in criteria)
    return (
        ("Problem Scoping", 15, "Task Framing"),
        ("Profiling Rigor", 25, "Profiling Evidence", "Hot Path Identification"),
        ("System-Specific Analysis", 20),
        ("Optimization Quality", 20, "Optimization Recommendations"),
...
def _compose_review_doc(
    ...
    lines.extend(["", "## Senior Engineer Notes", senior_notes.strip() or "The optimization investigation is ready for handoff."])
    return "\n".join(lines)
...
def _parse_review_doc_or_raise(review_doc: str, review_criteria: tuple[tuple, ...]) -> dict[str, Any]:
    ...
    if len(parsed_scores) != len(review_criteria):
        raise ValueError("Reviewer LLM did not return the full optimization criterion assessment set.")
...
def _parse_review_json(data: dict[str, Any], review_criteria: tuple[tuple, ...]) -> dict[str, Any]:
    max_score_lookup = {name: weight for name, weight, *_ in review_criteria}
    parsed_scores: list[CriterionAssessment] = []
    seen_criteria: set[str] = set()
...
def _blocked_review_response(
    *,
    ...
    improvement_action = "Enable the reviewer LLM and rerun optimization investigation review so the scoring assessments come from LLM output."
    ...
    def review(state: ReviewerState) -> dict[str, Any]:
        ...
        if not reviewer_llm.is_enabled():
            return _blocked_review_response(
                ...
                metadata=metadata,
                review_round=review_round,
                reason="Reviewer LLM is unavailable, so optimization investigation assessments cannot be generated.",
                loop_status="llm-unavailable",
            )

        review_criteria = _get_review_criteria(state)
        optimization_domain = str(state.get("optimization_domain", "optimization"))

        review_instructions = (
            f"You are a strict senior engineer reviewing an optimization investigation brief for {optimization_domain}. "
            "Score it hard against the following criteria: "
            f"{', '.join(f'{name} ({weight} pts)' for name, weight, *_ in review_criteria)}. "
            "Focus on evidence of actual performance hotspots — profiling data, call frequency, memory analysis, "
            "or measurable cost breakdowns. Speculation alone is not sufficient for approval. "
            "Check whether the investigator provided concrete numbers (ms, counts, bytes) or only qualitative descriptions. "
            ...
            "pass that independently re-validates the performance hypothesis with fresh evidence or measurement. "
        ...
        review_input = (
            f"Task prompt:\n{state['task_prompt']}\n\n"
            f"Optimization domain: {optimization_domain}\n"
            ...
            "Act like a demanding senior performance engineer who cares about evidence-based optimization, "
            "measurable impact, regression risk, and verification quality."
        )
        try:
            try:
                ...
                review_result = _parse_review_json(review_json, review_criteria)
                ...
                generated_review = reviewer_llm.generate_text(
                    instructions=(
                        ...
                        "# Optimization Investigation Review, Decision: APPROVE or REVISE, Overall Score: NN/100, ## Criterion Scores, ## Blocking Issues, "
                ...
                review_result = _parse_review_doc_or_raise(generated_review, review_criteria)
        except (LLMError, ValueError) as exc:
            return _blocked_review_response(
                artifact_path=artifact_path,
                metadata=metadata,
                review_round=review_round,
                reason=f"Reviewer LLM failed to produce usable optimization investigation assessments: {exc}",
        ...
        summary = (
            f"{metadata.name} approved optimization investigation review round {review_round} with score {progress.score}/100."
