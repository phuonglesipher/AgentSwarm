...
class LoopingInvestigationLLMClient:
    ...
    def generate_text(self, *, instructions: str, input_text: str, effort: str | None = None) -> str:
        ...
        if "Investigate the host project root like a senior engineer" in instructions:
            ...
            if self.investigation_calls == 1:
                return "\n".join(
                    [
                        ...
                        "- Need to search for external callers of regain_control().",
                        "",
            ...
            if "Previous reviewer feedback" not in input_text or "Blocking Issues" not in input_text:
            ...
            return "\n".join(
                [
                    ...
                    "",
                    "## Consumer & Caller Analysis",
                    "- Grepped the project for references to regain_control() outside src/player_movement.py.",
                    "- Only tests/test_player_movement.py calls it. No external consumers found.",
                    "- This means the fix scope is fully contained within the movement module.",
                    "",
                    "## Root Cause Hypothesis",
        ...
        if "strict senior engineer reviewing an investigation brief" in instructions:
            ...
            if self.review_calls == 1:
                return "\n".join(
                    [
                        ...
                        "",
                        "## Criterion Scores",
                        "- Focus: 16/20 - The brief has a direction but still feels broad.",
                        "- Evidence & Ownership: 15/25 - It names one file, but ownership is not grounded enough.",
                        "- Architecture: 11/15 - Some structure is present, but the system boundary needs more precision.",
                        "- Clean Code: 7/10 - Maintainability is mentioned, not argued.",
                        "- Optimization: 9/10 - Optimization non-goals are clear enough.",
                        "- Verification: 14/20 - Validation is too thin for confidence.",
                        "",
                        "## Blocking Issues",
            ...
            return "\n".join(
                [
                    ...
                    "",
                    "## Criterion Scores",
                    "- Focus: 19/20 - The scope is tight and actionable.",
                    "- Evidence & Ownership: 23/25 - Ownership is grounded in the runtime file, docs, and tests.",
                    "- Architecture: 14/15 - The runtime boundary and handoff are clear.",
                    "- Clean Code: 10/10 - Change-safety and maintainability concerns are explicit.",
                    "- Optimization: 9/10 - Performance is treated as a non-goal unless evidence appears.",
                    "- Verification: 18/20 - The validation plan is now concrete.",
                    "",
                    "## Blocking Issues",
                    "- None.",
                    "",
        ...
        if "Investigate the host project root like a senior engineer" in instructions:
            return "\n".join(
                [
                    ...
                    "",
                    "## Consumer & Caller Analysis",
                    "- Searched for callers of regain_control() across the codebase.",
                    "- Only tests/test_player_movement.py calls it externally. No other consumers found.",
                    "",
                    "## Root Cause Hypothesis",
        ...
        if "strict senior engineer reviewing an investigation brief" in instructions:
            return "\n".join(
                [
                    ...
                    "",
                    "## Criterion Scores",
                    "- Focus: 20/20 - The scope is tight and actionable.",
                    "- Evidence & Ownership: 25/25 - The likely owner and test surface are grounded in the project files.",
                    "- Architecture: 15/15 - The runtime boundary is clear.",
                    "- Clean Code: 10/10 - Maintainability concerns are explicit.",
                    "- Optimization: 10/10 - Optimization is handled proportionally.",
                    "- Verification: 20/20 - Validation steps are concrete.",
                    "",
                    "## Blocking Issues",
        ...
        if "Investigate the host project root like a senior engineer" in instructions:
            return "\n".join(
                [
                    ...
                    "",
                    "## Consumer & Caller Analysis",
                    "- Searched for callers of regain_control() and movement-related APIs across the project.",
                    "- Only tests/test_player_movement.py calls regain_control(). No external consumers found.",
                    "",
                    "## Root Cause Hypothesis",
        ...
        if "strict senior engineer reviewing an investigation brief" in instructions:
            return "\n".join(
                [
                    ...
                    "",
                    "## Criterion Scores",
                    "- Focus: 20/20 - The investigation is tightly scoped.",
                    "- Evidence & Ownership: 25/25 - Ownership is grounded in project files and tests.",
                    "- Architecture: 15/15 - The boundary and handoff are clear.",
                    "- Clean Code: 10/10 - Maintainability concerns are explicit.",
                    "- Optimization: 10/10 - Optimization is treated proportionally.",
                    "- Verification: 20/20 - Verification is concrete and technical.",
                    "",
                    "## Blocking Issues",
