...
- Keep blueprint folders in lowercase kebab-case, for example `gameplay-engineer-blueprint/`.
- Each blueprint must expose `build_graph(...)` from `entry.py`.
- Prefer small, focused helper functions in `core/` and blueprint modules. Keep comments brief and only where logic is non-obvious.
...
- When fixing blueprint runtime behavior, add or update a regression test in `tests/test_runtime.py`.
...
- If you add a new blueprint-specific model profile, wire it through `core/llm.py` and document the env vars in `.env.example`.
